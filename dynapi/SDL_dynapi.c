...
static SDL_Window *my_SDL_CreateWindow(const char *title, int x, int y, int w,
                                       int h, Uint32 flags) {
    ...
    if (!pool_created) {
        ...
        for (int i = 0; i < BUFFER_WINDOW_COUNT; i++) {
            egl_seats[i].window = real_SDL_CreateWindow(
                "Metro EGL workaround", SDL_WINDOWPOS_UNDEFINED,
                SDL_WINDOWPOS_UNDEFINED, 5, 5,
...
static SDL_GLContext my_SDL_GL_CreateContext(SDL_Window *w) {
    ...
    if (ctx) {
        ...
        if (!gl_primary.initialized) {
            ...
            LOG_FPRINTF(
                stderr,
                "[hook] primary GL context recorded (anchor): win=%p ctx=%p\n",
                (void *)w, ctx);
        } else {
            gl_primary.window = w;
            gl_primary.context = ctx;
            LOG_FPRINTF(stderr,
                        "[hook] additional GL context forced into share group: "
                        "win=%p ctx=%p\n",
                        (void *)w, ctx);
        }
        gl_unlock();
...
static int SDLCALL wrapped_thread_fn(void *arg) {
    ...
    if (jump_table.SDL_GL_MakeCurrent(window, shared_ctx) != 0) {
        ...
        LOG_FPRINTF(
            stderr,
            "[hook] shared GL context current on thread: win=%p ctx=%p\n",
...
static SDL_Thread *my_SDL_CreateThread(int(SDLCALL *fn)(void *),
    ...
    LOG_FPRINTF(stderr,
                "[hook] SDL_CreateThread(\"%s\"): injecting shared GL ctx=%p "
                "(share group of caller ctx=%p)\n",
    ...
    for (int i = 0; i < MAX_JOYSTICKS; i++) {
        if (!joy_table[i].joy) {
            ...
            SDL_Joystick *real_joy =
                jump_table.SDL_GameControllerGetJoystick(gc);
...
static void my_SDL_JoystickClose(SDL_Joystick *joy) {
    ...
    if (e) {
        if (e->gc)
            jump_table.SDL_GameControllerClose(e->gc);
        e->joy = NULL;
        e->gc = NULL;
        e->device_index = -1;
    }
...
static Uint8 my_SDL_JoystickGetHat(SDL_Joystick *joy, int hat) {
    ...
    if (jump_table.SDL_GameControllerGetButton(gc,
    ...
    if (jump_table.SDL_GameControllerGetButton(gc,
        ...
        val |= SDL_HAT_LEFT;
    if (jump_table.SDL_GameControllerGetButton(
            gc, SDL_CONTROLLER_BUTTON_DPAD_RIGHT))
        val |= SDL_HAT_RIGHT;
    return val;
}
...
static int my_SDL_PollEvent(SDL_Event *event) {
    for (;;) {
        ...
        case SDL_JOYDEVICEADDED: {
            ...
            if (!jump_table.SDL_IsGameController(idx))
                continue;
        ...
        case SDL_JOYDEVICEREMOVED: {
            ...
            for (int i = 0; i < MAX_JOYSTICKS; i++) {
                if (joy_table[i].joy && jump_table.SDL_JoystickInstanceID(
                                            joy_table[i].joy) == iid) {
                    LOG_FPRINTF(stderr,
                                "[hook] hotplug REMOVED: iid=%d gc=%p\n", iid,
                                (void *)joy_table[i].gc);
                    if (joy_table[i].gc) {
...
static void (*my_glRenderbufferStorage)(GLenum, GLenum, GLsizei,
                                        GLsizei) = NULL;
static void (*my_glRenderbufferStorageMultisample)(GLenum, GLsizei, GLenum,
                                                   GLsizei, GLsizei) = NULL;
static void (*my_glFramebufferRenderbuffer)(GLenum, GLenum, GLenum,
                                            GLuint) = NULL;
static GLenum (*my_glCheckFramebufferStatus)(GLenum) = NULL;
static void (*my_glBlitFramebuffer)(GLint, GLint, GLint, GLint, GLint, GLint,
                                    GLint, GLint, GLbitfield, GLenum) = NULL;
...
static SDL_Rect GetOpenGLLogicalScalingViewport(int pw, int ph) {
    SDL_Rect r;
    float want =
        (float)OpenGLLogicalScalingWidth / (float)OpenGLLogicalScalingHeight;
...
static int InitializeOpenGLScaling(int logical_w, int logical_h) {
    ...
    if (OpenGLLogicalScalingFBO) {
        ...
        if (OpenGLLogicalScalingMultisampleFBO) {
            ...
            OpenGLLogicalScalingMultisampleFBO =
                OpenGLLogicalScalingMultisampleColor =
    ...
    if (OpenGLLogicalScalingSamples > 0)
        my_glRenderbufferStorageMultisample(
            GL_RENDERBUFFER, OpenGLLogicalScalingSamples,
            alpha_size > 0 ? GL_RGBA8 : GL_RGB8, logical_w, logical_h);
    else
        my_glRenderbufferStorage(GL_RENDERBUFFER,
                                 alpha_size > 0 ? GL_RGBA8 : GL_RGB8, logical_w,
                                 logical_h);
    my_glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
                                 GL_RENDERBUFFER, OpenGLLogicalScalingColor);
    ...
    if (depth_size > 0 || stencil_size > 0) {
        ...
        if (OpenGLLogicalScalingSamples > 0)
            ...
            my_glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8,
                                     logical_w, logical_h);
        if (depth_size > 0)
            my_glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT,
                                         GL_RENDERBUFFER,
                                         OpenGLLogicalScalingDepth);
        if (stencil_size > 0)
            my_glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_STENCIL_ATTACHMENT,
                                         GL_RENDERBUFFER,
                                         OpenGLLogicalScalingDepth);
    }
    my_glBindRenderbuffer(GL_RENDERBUFFER, 0);

    if (my_glCheckFramebufferStatus(GL_FRAMEBUFFER) !=
    ...
    if (override_w > 0 && override_h > 0 && is_fullscreen &&
        ...
        if (scaling_context_initialized && OpenGLLogicalScalingFBO) {
            ...
            my_glBlitFramebuffer(0, 0, override_w, override_h, 0, 0, override_w,
                                 override_h, GL_COLOR_BUFFER_BIT, GL_NEAREST);
            ...
            my_glBlitFramebuffer(0, 0, override_w, override_h, dstrect.x,
                                 dstrect.y, dstrect.x + dstrect.w,
                                 dstrect.y + dstrect.h, GL_COLOR_BUFFER_BIT,
                                 filter);

            my_glClearColor(cc[0], cc[1], cc[2], cc[3]);
            if (has_scissor)
                my_glEnable(GL_SCISSOR_TEST);
            my_glBindFramebuffer(GL_FRAMEBUFFER, 0);
        }
    }
...
static SDL_bool my_SDL_GetWindowWMInfo(SDL_Window *window,
                                       SDL_SysWMinfo *info) {
    return SDL_FALSE;
}
...
static void *my_SDL_GL_GetProcAddress(const char *proc) {
    ...
    if (strcmp(proc, "glShaderSource") == 0) {
         ...
         * Mesa uses an ad-hoc binary-name workaround; we fix the offending
         * shader instead. */
...
static void apply_vibration_patches(void) {
    ...
    *(uintptr_t *)(jmp_activate + 2) = (uintptr_t)my_activate_vibration;
    ...
    uint8_t jmp_deactivate[12] = {0x48, 0xb8, 0, 0, 0,    0,
                                  0,    0,    0, 0, 0xff, 0xe0};
    ...
    *(uintptr_t *)(jmp_set_game + 2) = (uintptr_t)my_set_vibration_game;
    ...
    uint8_t jmp_set_camera[12] = {0x48, 0xb8, 0, 0, 0,    0,
                                  0,    0,    0, 0, 0xff, 0xe0};
...
#undef SDL_DYNAPI_PROC
    ...
    if (!gl_primary_lock) {
        fputs("[hook] gl_primary_lock failed\n", stderr);
        exit(1);
    }

    egl_seat_lock = jump_table.SDL_CreateMutex();
    if (!egl_seat_lock) {
        fputs("[hook] egl_seat_lock failed\n", stderr);
        exit(1);
    }
